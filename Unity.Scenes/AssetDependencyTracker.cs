...
namespace Unity.Scenes
{
    ...
    internal class AssetDependencyTracker<T> : IDisposable where T : unmanaged, IEquatable<T>
    {
        ...
        public void Add( GUID asset, T userKey, bool async, FixedString512Bytes loadDir = default )
        {
            ...
            var value = new ReportedValue
                { ReportedHash = default, UserKey = userKey, Async = async, LoadDir = loadDir, DidReport = false };
            ...
            do
            {
                if ( !value.DidReport || value.ReportedHash != artifact )
                {
                    completed.Add( new Completed
                        { Asset = guid, UserKey = value.UserKey, ArtifactID = artifact, LoadDir = value.LoadDir } );
