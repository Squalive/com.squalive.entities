...
namespace Unity.Scenes
{
    ...
    public partial struct SceneSystem : ISystem, ISystemStartStop
    {
        ...
        static internal RequestSceneLoaded CreateRequestSceneLoaded( FixedString512Bytes loadDir, LoadParameters loadParameters )
        {
            var requestSceneLoaded = new RequestSceneLoaded { LoadDir = loadDir, LoadFlags = loadParameters.Flags};
        ...
        internal static readonly SharedStatic<FixedString512Bytes> SceneLoadDir = SharedStatic<FixedString512Bytes>.GetOrCreate<SceneLoadDirKey>(  );
        
        private class SceneLoadDirKey{}
        ...
        public void OnStartRunning(ref SystemState state)
        {
            ...
            SceneLoadDir.Data = Application.streamingAssetsPath;
        }
        ...
        {
            var fullSceneInfoPath = EntityScenesPaths.FullPathForFile(Application.streamingAssetsPath, EntityScenesPaths.RelativePathForSceneInfoFile);
        ...
        public static Entity LoadSceneAsync( WorldUnmanaged world, Hash128 sceneGUID, LoadParameters parameters = default )
        {
            ...
            return LoadEntitySceneAsync( SceneLoadDir.Data, world, sceneGUID, parameters );
        }
        
        public static Entity LoadSceneAsync( FixedString512Bytes loadDir, WorldUnmanaged world, Hash128 sceneGUID, LoadParameters parameters = default )
        {
            if ( !sceneGUID.IsValid )
            {
                Debug.LogError( $"LoadSceneAsync - Invalid sceneGUID." );
            ...
            return LoadEntitySceneAsync( loadDir, world, sceneGUID, parameters );
        }

        private static Entity LoadEntitySceneAsync( FixedString512Bytes loadDir, WorldUnmanaged world, Hash128 sceneGUID, LoadParameters parameters )
        {
            ...
            if ( sceneEntity == Entity.Null )
                return CreateSceneEntity( loadDir, world, sceneGUID, parameters );

            LoadEntitySceneAsync( loadDir, world, sceneEntity, parameters );
            return sceneEntity;
        }

        static void LoadEntitySceneAsync( FixedString512Bytes loadDir, WorldUnmanaged world, Entity sceneEntity, LoadParameters parameters )
        {
            var requestSceneLoaded = CreateRequestSceneLoaded( loadDir, parameters );
            ...
            {
                var resolvedSectionEntities = world.EntityManager.GetBuffer<ResolvedSectionEntity>( sceneEntity )
                    .ToNativeArray( Allocator.Temp );
                for ( int i = 0; i < resolvedSectionEntities.Length; i++ )
                    world.EntityManager.AddComponentData( resolvedSectionEntities[ i ].SectionEntity,
                        requestSceneLoaded );
            }
        }
        ...
        {
            LoadEntitySceneAsync( SceneLoadDir.Data, world, sceneEntity, parameters );
        }

        static Entity CreateSceneEntity( FixedString512Bytes loadDir, WorldUnmanaged world, Hash128 sceneGUID, LoadParameters parameters = default )
        {
            var requestSceneLoaded = CreateRequestSceneLoaded( loadDir, parameters );
