        ...
        protected override unsafe void OnUpdate()
        {
            ...
            if (!m_AddScenes.IsEmptyIgnoreFilter)
            {
                ...
                using (var addScenes = m_AddScenes.ToEntityArray(Allocator.TempJob))
                {
                    ...
                    for (int i = 0; i != addScenes.Length; i++)
                    {
                        ...
                        _AssetDependencyTracker.Add( guid, sceneEntity, async, requestSceneLoaded.LoadDir );
                        if (requireRefresh)
            ...
            foreach (var change in _Changed)
            {
                ...
                if (change.ArtifactID != default)
                {
                    ...
                    SceneHeaderUtility.ScheduleHeaderLoadOnEntity( EntityManager, change.UserKey, scene.SceneGUID, request, change.ArtifactID, change.LoadDir.Value );
                }
                else
                    Debug.LogError(
            ...
            Entities.WithStructuralChanges().WithNone<DisableSceneResolveAndLoad, ResolvedSectionEntity>().ForEach(
                    ...
                    ref ResolvedSceneHash resolvedSceneHash, ref RequestSceneLoaded requestSceneLoaded ) =>
                {
                    ...
                    var headerLoadResult = SceneHeaderUtility.FinishHeaderLoad( requestHeader, scene.SceneGUID,
                        requestSceneLoaded.LoadDir.Value );
                    ...
                    ResolveSceneSectionUtility.ResolveSceneSections( EntityManager, sceneEntity, requestSceneLoaded,
                        ...
                        m_ResolveSceneSectionArchetypes, headerLoadResult.SectionPaths,
    ...
    partial class ResolveSceneReferenceSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            Entities.WithStructuralChanges().WithNone<DisableSceneResolveAndLoad, ResolvedSectionEntity, RequestSceneHeader>().
                ...
                {
                    SceneHeaderUtility.ScheduleHeaderLoadOnEntity( EntityManager, sceneEntity, scene.SceneGUID,
                        requestSceneLoaded, default, requestSceneLoaded.LoadDir.Value );
                }).Run();
            ...
            Entities.WithStructuralChanges().WithNone<DisableSceneResolveAndLoad, ResolvedSectionEntity>().ForEach(
                    ...
                    ref ResolvedSceneHash resolvedSceneHash, ref RequestSceneLoaded requestSceneLoaded ) =>
                {
                    ...
                    var headerLoadResult =
                        SceneHeaderUtility.FinishHeaderLoad( requestHeader, scene.SceneGUID, requestSceneLoaded.LoadDir.Value );
                    if ( !headerLoadResult.Success )
                    {
                        requestHeader.Dispose();
                        EntityManager.AddBuffer<ResolvedSectionEntity>( sceneEntity );
