...
namespace Unity.Entities.Tests
{
    public class PatchingEndToEndWithSerialization : EntityDifferTestFixture
    {
        ...
        static void AssertChangeSetsAreEqual(LiveConversionChangeSet srcChange, LiveConversionChangeSet dstChange)
        {
            ...
            Assert.AreEqual(srcChange.Changes.SetManagedComponents.Length,
                dstChange.Changes.SetManagedComponents.Length);
        }
        ...
        public void CanSerializeAndDeserializeManagedComponents()
        {
            using (var differ = new EntityManagerDiffer(SrcWorld.EntityManager, SrcWorld.UpdateAllocator.ToAllocator))
            {
                ...
                var dstManagedComponent =
                    GetManagedComponentData<EcsTestManagedDataEntity>(DstEntityManager, entityGuid);
                ...
                Assert.That(dstManagedComponent.nullField.ClassWithString,
                    Is.Not.SameAs(srcManagedComponent.nullField.ClassWithString));
                Assert.That(dstManagedComponent.nullField.ClassWithString.String,
                    Is.EqualTo(srcManagedComponent.nullField.ClassWithString.String));
